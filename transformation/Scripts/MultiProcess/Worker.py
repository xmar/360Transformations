...
def WorkerManager(GenericWorker, shared_job_q, shared_result_q, exit_event):
    ...
    proc = mp.Process(target=GenericWorker,
...
def GenericWorker(SpecializedWorker, outDir, hostname, shared_job_q, shared_result_q):
...
def FixedBitrateAndFixedDistances(trans, config, args):
    ...
    (job, n, inputVideo, maxIteration, reuseVideo, outputResolution, k, nbQec, bitrateGoal, distStep, outputDir) =\
        args
    workDone = False
    averageGoalSize = (0,0)
    ...
    try:
        ...
        workDone = True
        return (workDone, Results(outputDir, job))
