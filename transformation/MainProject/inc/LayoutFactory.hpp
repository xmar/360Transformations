    ...
    public:
        KeyRotationDescription(ConfigParserBase* lcpb, std::string label, std::string description, bool optional, Quaternion defVal = Quaternion(1)):
            KeyTypeDescription<std::string>(lcpb, label,
                    "(json) "+description+" format: \e[2mif type is \"euler\" you have to give the yaw, pitch and roll angle in degree {\"type\":\"euler\", \"yaw\":0.0, \"pitch\":0.0, \"roll\":0.0} ; Euler angles can be given with \"euler\" type, \"eulerXYZ\" or \"eulerZYX\". \"euler\" and \"eulerXYZ\" are identical; else if type is \"quaternion\" you have to give the w, x, y, z value of the quaternion that represente the rotation. A normalization is performed by the software. {\"type\":\"quaternion\", \"w\":1.0, \"x\":0.0, \"y\":0.0, \"z\":0.0} ; else if type is \"angleAxis\" you have to give an \"angle\" in degree and a direction vector x, y, z that generates the rotation axis (this vector should not be (0, 0, 0)). {\"type\":\"angleAxis\", \"angle\":90, \"x\":0, \"y\":0, \"z\":1}\e[0m",
                    optional, ""),
            m_defVal(defVal)
        {}
