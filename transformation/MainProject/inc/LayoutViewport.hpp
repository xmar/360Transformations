...
class LayoutViewport: public Layout
{
    public:
        ...
        LayoutViewport(DynamicPosition dynamicPosition, unsigned int width, unsigned int height, double horizontalAngleOfVision, double verticalAngleOfVision): Layout(width,height, std::make_shared<VectorialTrans>()),
...
class LayoutConfigParserViewport: public LayoutConfigParser
{
    ...
    protected:
        std::shared_ptr<Layout> CreateImpl(std::string layoutSection, pt::ptree& ptree) const override
        {
            ...
            return std::make_shared<LayoutViewport>(std::move(dynamicPosition), m_width.GetValue(layoutSection, ptree), m_height.GetValue(layoutSection, ptree), m_horizontalAngleOfVision.GetValue(layoutSection, ptree), m_verticalAngleOfVision.GetValue(layoutSection, ptree));
        }
    private:
