...
\end{tikzpicture}\caption{\ac{FoV}-adaptive streaming system: the server offers \textit{n*k} representations (\textit{n} QECs at \textit{k} bit-rates). The streams lasts three segments from $s_1$ to $s_3$. The client moves the head from left to right, while the available bandwidth varies. For each segment, the client requests a representation that matches both the \ac{FoV} and the throughput.}
