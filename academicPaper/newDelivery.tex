...
\tikzset{
	...
	% fifth paramerer is the y pos of QEC
		code={
			...
			\fi
			\ifnum#2>0
				\node[font=\footnotesize,anchor=east] at (legbwme.west) {QEC$_#2$};
			\fi
		}
	}
}
...
\pic[local bounding box=leftguy] at (4.5,-2.75) {leftVR=1.5};
...
\pic[local bounding box=oneseg] at ([yshift=-4pt]client|-mpd.south) {reqresp=
	{QEC$_2$ med}
	{\pic at (0,0) {equirec={\sizeMed}{-2}{-1}};\\}
	};
\pic[local bounding box=twoseg] at ([yshift=-4pt]client|-oneseg.south) 
	{reqresp={QEC$_3$ high}
	{\pic at (0,0) {equirec={\sizeBig}{0}{0}};\\}
	};
\pic[local bounding box=thirdseg] at ([yshift=-4pt]client|-twoseg.south) 
	{reqresp={QEC$_1$ low}
...
\end{tikzpicture}
\caption{\ac{FoV}-adaptive streaming system: the server offers nine representations (three QECs at three bit-rates). Here we show three segments of the video stream. The client moves the head from left to right, while the available bandwidth varies. For each segment, the client requests a representation that matches both the \ac{FoV} and the throughput.}
