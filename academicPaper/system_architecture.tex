...
bit-rate and \ac{QEC} \XC{position}{}), enabling the client to regularly switch
...
often denoted by \emph{pitch}, \emph{yaw}, and \emph{roll}.}{} Changes
...
\parag{Adaptation algorithm} Similarly to \ac{DASH}, the client runs an
...
the next $t$ seconds (\textit{i.e.}, $t$ being the segment \XC{duration}). It
...
size of the radius.}{}} to the \ac{FoV} center at the time the client
runs the adaptation algorithm.
%Most of these algorithms focus on predicting the expected throughput for the next few seconds in order to make optimal bit-rate decisions.
