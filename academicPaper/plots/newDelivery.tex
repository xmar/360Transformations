...
\end{tikzpicture}\caption{\ac{FoV}-adaptive streaming system: the server offers $n*k$ representations ($n$ QECs at $k$ bit-rates). The streaming session lasts for three segments from $s_1$ to $s_3$. The client moves his/her head from left to right, while his/her available bandwidth varies. For each segment, the client requests a representation that matches both the \ac{FoV} and the network throughput.}
