...
% ===== client
\node[element,left=\ecartElement of server] (client) {};
...
\tikzset{every loop/.style={in=0,out=30,looseness=5}}

\path[->] (client) edge [loop] node[above, near start] {updateFoV()} (client);
\draw[-latex] (client.-15) to[bend left=30] node[below, outer sep=8pt] {getSegment(FoV)} (server.195);
\draw[-latex] (server) to[bend left=30] node[below] {videoVersion}(client);
...
% ===== server
\node[element,right=\ecartElement of client] (server) {};
...
\end{tikzpicture}
\caption{360-degree video delivery system architecture}
