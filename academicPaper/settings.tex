...
re-projecting these videos to another layout. \XC{Our tool rotate the
...
\begin{itemize}[leftmargin=7pt, itemindent=0pt, topsep=2pt, itemsep=0pt]
   ...
   \item \emph{The original video at full quality}. \XC{It is from
   this video we extract the reference viewports, which
   are used to compute the objective quality metrics. We used the
   ``\emph{Bridge
   jumping}''\footnote{\url{https://youtu.be/yarcdW91djQ}} video. We
   can extract viewports at $1080$p resolution from this 4K
   equirectangular video. For the objective video quality assessment
   tools we used: \ac{MS-SSIM}~\cite{wang2003multiscale} and
   \ac{PSNR}. \ac{MS-SSIM} compares image by image the structural
   ...
   decibel (\si[mode=text]{\decibel}), $+\infty$ being two identical videos.}{}. Since we
   compare several encoded versions of the same video against the
   original, these tools provide a fair performance evaluation.

   \item \emph{The same video re-encoded at a target bit-rate}. \XC{This
   video represents the basic delivered stream in a regular delivery
   system given a bit-rate target (here \SI{\testbitrateBudget}{\mega
   bps} being \testbitrateBudgetPercentage{} of the original video bit-rate).}{} We re-encoded the original full-quality video with \ac{HEVC}
...
arrangement for each layout. \XC{For cube map and pyramid, the
\ac{QEC} is located at the center of a face. This face is set at full
quality (same bit-rate budget as the same portion of the original
video), and the other faces at low quality (\SI{25}{\percent} of the
full quality target). For equirectangular with tiles, the \ac{QEC} is
...
\num{1.5}. \XC{Here the cube map projection has better results
...
the video codec exploits the same bit-rate budget.}{} We will see in
