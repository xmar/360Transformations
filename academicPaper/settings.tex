...
the evolution of the quality in the unique \ac{QER} of each representation). We will not
...
\begin{itemize}%[leftmargin=7pt, itemindent=0pt, topsep=2pt, itemsep=-3pt]
   ...
   \item \emph{The original equirectangular video at full quality}%
...
To assess the perceived quality, we extract a ten second viewport, at specific distance from the \ac{QEC}, at the same static position on the original equirectangular video and on the tested video. We used two objective video quality metrics to measure the quality of the extracted viewport compared to the original full quality viewport:
...
We perform multiple quality assessment (typically forty) at the same distance and average the result.
...
%We define a \textit{quality-differentiated layout} as the
...
%Here the cube map projection has better results
...
We study next the interplay between this distance, the segment length and the number of \acp{QER}.
...
users select a \ac{QER}. Then, we measure the orthodromic distance
...
match between the \ac{QER} and the \FoV{} center. However,
...
% The choice of the position of $n$ \ac{QEC} on a sphere is not
...
number $n$, then we determined the position of the $n$ \acp{QER}
...
between the \FoV{} center and the \ac{QER} that was chosen at the
...
%We used the best projection, \textit{i.e.} the cube map projection.
...
%
%is between $6$ and $7$.
%More \acp{QEC} improve slightly the quality for a linear growth in the
